...
class VisionCameraView(
  ...
  private val barcodeSetForRegex = mutableSetOf<BarcodeResult>()
...
//    imageScanner.multipleScanEnabled(visionViewState.isMultipleScanEnabled)
  ...
  private val cameraXBarcodeCallback = object : CameraXBarcodeCallback {
    ...
    override fun aggregatedBarcodeResults(barcodes: List<BarcodeResult>) {
      ...
      // We only need to show bounding boxes on Barcodes and QRCodes when either barcode or QRCode
      // mode is selected. In OCR and Photo mode, we will not draw them.
      if (visionViewState.detectionMode != DetectionMode.Barcode && visionViewState.detectionMode != DetectionMode.QRCode) {
        barcodeAndQRCodeBoundingBoxesMap.forEach {
          removeViewFun(it.value)
        }
        barcodeAndQRCodeBoundingBoxesMap.clear()
      ...
      if (!focusSettings.showCodeBoundariesInMultipleScan || !(visionViewState.detectionMode == DetectionMode.Barcode || visionViewState.detectionMode == DetectionMode.QRCode))
        return

      val barcodesToDrawBoxOn = when (visionViewState.detectionMode) {
        DetectionMode.Barcode -> filterByAppliedTemplate(onlyBarcodes) // If a template is applied, filter barcode with that template too.
        DetectionMode.QRCode -> onlyQRCodes
        DetectionMode.OCR, DetectionMode.PriceTag, DetectionMode.Photo -> throw IllegalStateException("If detectionMode is set to OCR or Photo, then this whole section should have been skipped. This is an issue in SDK and needs fixing immediately.")
      }
    ...
    override fun onBarcodesScanned(barcodes: List<BarcodeResult>) {
      ...
      // If nothing is being detected, then clear the barcodeSetForRegex because it means that
      // the camera has no barcode or QRCode in preview.
      if (barcodes.isEmpty()) {
        barcodeSetForRegex.clear()
        ...
        DetectionMode.Barcode -> {
          if (onlyBarcodes.isNotEmpty()) {
            if (visionViewState.scanningMode == ScanningMode.Auto || (visionViewState.scanningMode == ScanningMode.Manual && isCaptureButtonClicked)) {
              ...
              scannerCallback?.onBarcodesDetected(onlyBarcodes)
            }
          }
        }

        DetectionMode.QRCode -> {
          if (onlyQRCodes.isNotEmpty()) {
            if (visionViewState.scanningMode == ScanningMode.Auto || (visionViewState.scanningMode == ScanningMode.Manual && isCaptureButtonClicked)) {
              stopScanning()
              isCaptureButtonClicked = false
              scannerCallback?.onBarcodesDetected(onlyQRCodes)
            }
          }
        }

        DetectionMode.OCR, DetectionMode.PriceTag, DetectionMode.Photo -> {
          // Nothing to do in case of OCR
        }
      }

      barcodeSetForRegex.clear()
      barcodeSetForRegex.addAll(barcodes)
    }
  ...
  private fun terminateDocumentAutoCapture() {
    autoCaptureDocumentTimer?.cancel() ?: return
    autoCaptureDocumentTimer = null
    Log.d(TAG, "Document auto capture cancelled")
  }

  private fun getBarcodesWithinFocusFrame(barcodes: List<BarcodeResult>) = if (focusSettings.isFrameAddedAndApplicable()) {
    barcodes.filter {
  ...
  private fun captureImage(onCaptureCompletion: (() -> Unit)? = null) {
    imageCaptureUseCase?.takePicture(
      ...
      object : ImageCapture.OnImageCapturedCallback() {
        override fun onCaptureSuccess(imageProxy: ImageProxy) {
          ...
          (context as Activity).runOnUiThread {
            ...
            scannerCallback?.onImageCaptured(bitmap, newImageFile, barcodeSetForRegex.toList())
          }
          imageProxy.close()
          onCaptureCompletion?.invoke()
        }
      ...
      }) ?: throw IllegalStateException("You need to call startCameraAndScanning() first.")
  }
