...
// PR #199 review (finding 5) — the numeric-id branch below appears to be unreachable,
// but `rampZoomRatio` is appended last anyway so the question doesn't have to be right.
// It was originally inserted mid-array (index 6), renumbering the ids of the five
// commands after it; an OTA JS/native version skew could then dispatch, say,
// `setFocusSettings` as whatever id the OLD native maps to `pauseDetection`. The trace
// below argues that can't happen; appending costs nothing and removes the dependency on
// that argument holding. Note the "Fabric sometimes sends commands as numbers" comment
// on the branch itself contradicts the trace — someone wrote it for a reason we could
// not recover, which is the other half of why this is append-only now.
// The trace, for the record:
//   - `Commands.<name>(ref, ...)` (src/specs/VisionCameraViewNativeComponent.ts, called
//     exclusively from src/VisionCamera.tsx) is generated by RN's codegenNativeCommands,
//     which always calls `dispatchCommand(ref, command, args)` with `command` as the
//     literal string name — never a numeric index (see codegenNativeCommands.js).
//   - On Fabric, that lands in RCTMountingManager's
//     `synchronouslyDispatchCommandOnUIThread:commandName:args:`, which forwards that
//     same string straight into `handleCommand:args:` unchanged — no numeric conversion
//     anywhere in the genuine-Fabric-component path (this class implements
//     RCTComponentViewProtocol directly; it is not a `RCTLegacyViewManagerInteropComponentView`,
//     which is the only place a numeric command id is ever actually looked up, and only
//     for OLD-architecture ViewManagers bridged into Fabric via that adapter).
// So `commandId != nil` should never evaluate true for this component via any JS call
// this SDK ships. The branch is kept, and the array is kept append-only, so that being
// wrong about that is harmless rather than a silent wrong-command dispatch.
- (void)handleCommand:(const NSString *)commandName args:(const NSArray *)args
{
  ...
  if (commandId != nil) {
    // Map command IDs to command names by position in the TS supportedCommands array
    // (src/specs/VisionCameraViewNativeComponent.ts). This ordering is a wire format:
    // an id resolved here against a different JS bundle's ordering dispatches the wrong
    // command silently. APPEND-ONLY — never insert mid-array, never reorder. Android's
    // receiveCommand matches by name, so it is unaffected either way.
    NSArray *commandNames = @[@"capture", @"stop", @"start", @"rescan", @"toggleFlash", @"setZoom", @"setFocusSettings", @"pauseDetection", @"resumeDetection", @"setTorchEnabled", @"setFocusPoint", @"rampZoomRatio"];
