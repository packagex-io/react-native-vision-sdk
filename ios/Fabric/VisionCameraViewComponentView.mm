...
// PR #199 review (finding 5) — verified UNREACHABLE, ordering left as-is on purpose.
// `rampZoomRatio` was inserted mid-array below (index 6), renumbering the ids of the
// five commands after it; the concern was that an OTA JS/native version skew could hit
// the numeric-id branch and dispatch, say, `setFocusSettings` as whatever id the OLD
// native maps to `pauseDetection`. Traced the actual dispatch path to rule this out:
...
// So `commandId != nil` never evaluates true for this component under normal operation —
// no version-skew scenario can reach it via any JS call this SDK ships. Left the mapping
// array (and the mid-array rampZoomRatio insertion) untouched rather than churn dead code.
